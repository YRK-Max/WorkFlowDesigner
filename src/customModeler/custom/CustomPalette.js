 ...
 * @param  {customPaletteProvider} provider
 *
 * @example
...
Palette.prototype._update = function () {
  ...
  forEach(entries, function (entry, id) {
    ...
    if (!container) {
      ...
      const arrowDown = 'el-icon-arrow-up'
      const groupLabel = domify(
        `<div class="groupLabel"><div class="folder-div"><a href="javascript:void(0)" class="img-folder"></a></div><span title="${grouping}">${grouping}</span><div class="img-div"><a href="javascript:void(0)" class="${arrowDown}"></a></div></div>`
      )

      groupLabel.addEventListener('click', function () {
        const iconArrowDown = this.querySelector('.el-icon-arrow-down')
        const iconArrowLeft = this.querySelector('.el-icon-arrow-up')
        if (iconArrowDown) {
          // const isArrowDown = Array.from(iconArrowDown).includes('el-icon-arrow-down')
          iconArrowDown.classList = ['el-icon-arrow-up']
    ...
    var html = entry.html || (
      entry.separator
        ? '<hr class="separator" />'
        : '<span class="entry" draggable="true" style="display: none">'+ entry.title +'</span>')
